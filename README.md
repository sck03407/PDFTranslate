...
- Windows Tauri 桌面安装包可由 GitHub Actions 直接打进同一套便携后端资源：嵌入式 Python、`pdf2zh_next`、依赖、BabelDOC 资源和配置模板；Tauri 只负责启动本地 FastAPI 后端并打开同一套 React WebUI。
...
├── .github/workflows/            # 手动发布 Windows 便携包、Tauri 安装包和 Docker 镜像
...
Windows 便携包和 Windows Tauri 安装包默认不启用登录，首次打开会直接进入单机管理员模式。若管理员在 `config/distribution.toml` 中设置 `require_gui_login = true`，请同时填写 `user_password` 和 `admin_password`；默认用户名仍为 `user` 和 `admin`，密码默认为空，不能作为正式登录密码使用。
...
- Windows 便携 zip。
- Windows 的一体 Tauri 桌面安装包，内置同一套便携 Python/FastAPI 后端资源。
- macOS / Linux 的 Tauri 桌面壳包；如需同等离线一体分发，需要补充对应平台的后端资源。
- GHCR Docker 镜像。

Windows Tauri 包优先启动安装包内的 `backend` resource；开发调试时仍可用 `PDFTRANSLATE_BACKEND_BIN`、`PDFTRANSLATE_RUNTIME_DIR` 或系统 `PATH` 中的 `pdf2zh` 覆盖。Docker、便携包和 Tauri 共用同一套 React WebUI，因此登录页和管理员设置页的布局一致；差异只来自后端配置是否要求登录。
...
- 输出目录按会话隔离，支持历史会话清理。
- GitHub Release 可同时发布便携 zip、Windows 一体 Tauri 安装包和 Docker 镜像。
