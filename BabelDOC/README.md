...
- This directory is the bundled BabelDOC engine source kept in the repository for a stable local build baseline.
- **Self-deployment**: the sibling `PDFMathTranslate-next/` application layer in this repository provides self-deployment + WebUI with more translation services.
...
[Supported Language](./docs/supported_languages.md)
...
>
> End users who need self-deployment should use the sibling `PDFMathTranslate-next/` application layer included in this repository.
> 
> If you find that an option is not listed below, it usually means the option is intended for debugging or internal validation. Please do not rely on undocumented options in a public release workflow.
...
> 
> If you need broader language-specific word-boundary handling, treat it as an upstream BabelDOC engine enhancement topic and document your local compatibility findings in this repository.
...
> [!TIP]
>
> 1. Currently, only OpenAI-compatible LLM is supported. For more translator support, please use the sibling `PDFMathTranslate-next/` application layer included in this repository.
...
The current recommended way to call BabelDOC in Python is to call the `high_level.do_translate_async_stream` function of the sibling `PDFMathTranslate-next/` application layer in this repository.
...
> [!NOTE]
>
> The API compatibility here mainly refers to the compatibility with the sibling `PDFMathTranslate-next/` application layer in this repository.
...
BabelDOC is included here as a bundled engine baseline. Bug reports,
reproducible PDFs, documentation fixes, and small compatibility fixes are
welcome in the context of this repository. For changes to parsing, rendering, translation, or service integration
behavior, please open an issue for discussion before submitting a pull request.

Everyone interacting with the code and documentation in this bundled engine directory is expected to follow the repository's code of conduct and contribution expectations.

[Immersive Translation](https://immersivetranslate.com) is part of the upstream BabelDOC ecosystem background. If you later publish your own contributor reward rules, replace this sentence with your repository's own policy.
