...
Execute the translation command in the command line to generate the translated document `example-mono.pdf` and the bilingual document `example-dual.pdf` in the current working directory. The current default fallback is a no-key basic translation service, while the GUI and CLI still let you switch to any supported engine. More supported translation services can be found in [Documentation of Translation Services](./Documentation-of-Translation-Services.md).
...
#### Fashion Translation Customization

This customized branch adds apparel-focused defaults while keeping the existing BabelDOC parsing, table handling, and PDF reconstruction pipeline intact.
...
- Replace the API settings in `fashion-online-high-quality.toml` with your own service values.
- `fashion-online-high-quality.toml` also loads `fashion-customer-glossary-template.csv` by default, so you can extend customer, brand, and fabric-composition terms in place.
- You can layer your own customer terms through `--glossaries`.
...
- For GitHub releases, use the root-level `.github/workflows/fashion-release.yml`. It can publish a portable zip, a Windows self-contained Tauri installer with the bundled portable backend resource, macOS/Linux Tauri shell bundles, and a GHCR Docker image from either the stable local line or the latest upstream `funstory-ai/BabelDOC` source.
- The Windows Tauri backend resource is generated from the same portable builder, so it includes embedded Python, `pdf2zh_next`, dependencies, BabelDOC offline assets, and config templates. This does not require changes inside BabelDOC; BabelDOC upgrades still flow through the packaging scripts by selecting a stable local checkout, latest upstream source, or a specific upstream ref.
- Docker, portable, and Tauri builds use the same React login and Settings pages. Whether the centered login panel appears is controlled by `require_gui_login` or an auth file; Docker enables login by default.
...
<!-- Note: System prompt support has changed across upstream revisions. Keep this section aligned with the current repository behavior. -->
