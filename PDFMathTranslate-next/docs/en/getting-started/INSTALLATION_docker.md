...
> - To publish a Docker image, Windows portable zip, and Tauri desktop bundles from GitHub Actions, use the root-level `.github/workflows/fashion-release.yml`. The Windows Tauri installer stages the same portable backend as a bundled resource, while macOS/Linux Tauri bundles still need platform-specific backend resources before they can be treated as self-contained offline installers.
<!--
