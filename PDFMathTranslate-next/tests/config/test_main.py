...
class TestConfigManager:
    ...
    def test_read_toml_file_accepts_utf8_bom(self, tmp_path: Path):
        """Test TOML config files with UTF-8 BOM can be parsed."""
        config_file = tmp_path / "config.v3.toml"
        config_file.write_text(
            '\ufeff[gui_settings]\nui_lang = "zh"\n', encoding="utf-8"
        )

        cm = ConfigManager()
        content = cm._read_toml_file(config_file)

        assert content["gui_settings"]["ui_lang"] == "zh"
    ...
    def test_end_to_end_config(
        ...
        with patch("argparse.ArgumentParser.parse_args", return_value=args):
            ...
            assert cm.settings.report_interval == 1.0  # From env var
            assert cm.settings.config_file == str(config_file.resolve())
            assert (
