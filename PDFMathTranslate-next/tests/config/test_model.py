...
from pdf2zh_next.config.translate_engine_model import CustomOpenAICompatibleSettings
...
class TestTranslationSettings:
    def test_default_values(self):
        ...
        assert settings.disable_builtin_fashion_glossary is False
        assert settings.disable_builtin_fashion_prompt is False
...
class TestCustomOpenAICompatibleSettings:
    def test_transform_to_openai_settings(self):
        settings = CustomOpenAICompatibleSettings(
            custom_openai_compatible_name="BridgeGroup Custom",
            custom_openai_compatible_model="qwen-plus",
            custom_openai_compatible_base_url="https://example.invalid/v1",
            custom_openai_compatible_api_key="test-key",
        )

        settings.validate_settings()
        transformed = settings.transform()

        assert transformed.openai_model == "qwen-plus"
        assert transformed.openai_base_url == "https://example.invalid/v1"
...
class TestCLIEnvSettingsModel:
    ...
    def test_non_llm_engine_disables_builtin_fashion_glossary(self):
    ...
    def test_siliconflowfree_disables_auto_term_extraction(self):
        settings = CLIEnvSettingsModel(
            siliconflowfree=True,
            translation={"no_auto_extract_glossary": False},
        ).to_settings_model()
        settings.validate_settings()
