...
def _extract_json_array_fragment(message: str) -> str:
    start = message.find("[")
    end = message.rfind("]")
...
class SiliconFlowFreeTranslator(BaseTranslator):
    ...
    def do_llm_translate(self, text, rate_limit_params: dict = None):
        ...
        try:
            ...
            logger.warning(
                "SiliconFlowFree returned non-JSON HTTP response during translation",
                exc_info=True,
            )
        ...
        if (
            ...
            message = _extract_json_array_fragment(message)
