...
from pdf2zh_next.config.model import DEFAULT_GUI_BRAND_NAME
from pdf2zh_next.config.model import DEFAULT_GUI_BRAND_URL
...
from pdf2zh_next.config.model import service_supports_builtin_fashion_glossary
...
from pdf2zh_next.fashion_defaults import normalize_glossary_files
from pdf2zh_next.fashion_defaults import restore_customer_glossary_template_rows
from pdf2zh_next.fashion_defaults import resolve_glossary_path
...
FASHION_GLOSSARY_SUPPORT_INDEX_MAP = {}
...
def _translate_gui_text(text: str | None) -> str | None:
    if text is None:
        return None
    return _(text)


def _service_supports_fashion_glossary(service_name: str | None) -> bool:
    return FASHION_GLOSSARY_SUPPORT_INDEX_MAP.get(service_name, False)


def _service_supports_llm(service_name: str | None) -> bool:
    return LLM_support_index_map.get(service_name, False)


def _resolve_selected_service(current_settings: CLIEnvSettingsModel) -> str | None:
...
def _get_default_customer_glossary_files(service_name: str | None) -> str | None:
    if not _service_supports_fashion_glossary(service_name):
        return None
    return str(ensure_default_customer_glossary_template())


def _get_default_gui_glossary_files(
    service_name: str | None,
    configured_glossaries: str | None,
) -> str | None:
    if not _service_supports_fashion_glossary(service_name):
        return None
    return combine_glossary_files(
        configured_glossaries,
        _get_default_customer_glossary_files(service_name),
    )


def _load_glossary_preview_rows(glossary_files: str | None) -> list[list[str]]:
    normalized_glossaries = normalize_glossary_files(glossary_files)
    if not normalized_glossaries:
        return []

    rows: list[list[str]] = []
    for glossary_file in normalized_glossaries.split(","):
        ...
        try:
            ...
            logger.warning(
                "Failed to load glossary preview from %s",
                glossary_path,
                exc_info=True,
            )
    return rows
...
def _load_customer_glossary_editor_rows() -> list[list[str]]:
    try:
        return load_customer_glossary_template_rows()
    except Exception:
        logger.warning(
            "Failed to load customer glossary template rows",
            exc_info=True,
        )
        return []


def _resolve_brand_name(brand_name: str | None) -> str:
    cleaned = (brand_name or "").strip()
    return cleaned or DEFAULT_GUI_BRAND_NAME


def _resolve_brand_url(brand_url: str | None) -> str:
    return (brand_url or "").strip()


def _build_brand_markdown(brand_name: str | None, brand_url: str | None) -> str:
    resolved_name = _resolve_brand_name(brand_name)
    resolved_url = _resolve_brand_url(brand_url)
    if resolved_url:
...
if not available_services:
    available_services = [x.translate_engine_type for x in TRANSLATION_ENGINE_METADATA[:1]]

assert available_services, "No translation service is enabled"

default_gui_service = DEFAULT_TRANSLATION_ENGINE_METADATA.translate_engine_type
if default_gui_service not in available_services:
    default_gui_service = available_services[0]

selected_gui_service = _resolve_selected_service(settings) or default_gui_service
default_gui_glossaries = _get_default_gui_glossary_files(
    selected_gui_service,
    settings.translation.glossaries,
)
if default_gui_glossaries:
    settings.translation.glossaries = default_gui_glossaries
...
def _build_translate_settings(
    ...
    uploaded_glossaries = ui_inputs.get("glossaries")
    save_auto_extracted_glossary = ui_inputs.get("save_auto_extracted_glossary")
    use_builtin_fashion_prompt = ui_inputs.get("use_builtin_fashion_prompt", True)
    use_builtin_fashion_glossary = ui_inputs.get("use_builtin_fashion_glossary", True)
    configured_glossaries = normalize_glossary_files(
        translate_settings.translation.glossaries
    )
    if configured_glossaries:
        configured_glossaries = combine_glossary_files(
            *[
                str(
                    resolve_glossary_path(
                        glossary_file,
                        config_file=translate_settings.config_file,
                    )
                )
                for glossary_file in configured_glossaries.split(",")
            ]
        )
    default_customer_glossaries = _get_default_customer_glossary_files(service)
    runtime_glossaries = None
    persisted_glossaries = combine_glossary_files(
        configured_glossaries,
        default_customer_glossaries,
    )
    if _service_supports_fashion_glossary(service):
        runtime_glossaries = combine_glossary_files(
            configured_glossaries,
            default_customer_glossaries,
            uploaded_glossaries,
        )
    ...
    translate_settings.translation.disable_builtin_fashion_glossary = (
        not use_builtin_fashion_glossary
    )
    ...
    translate_settings.translation.glossaries = runtime_glossaries
...
def save_customer_glossary_template(glossary_rows):
    try:
        saved_path = save_customer_glossary_template_rows(glossary_rows)
        saved_rows = load_customer_glossary_template_rows()
        message = _("Configuration saved to: {path}").format(path=saved_path)
        return gr.update(value=saved_rows), gr.update(value=message, visible=True)
    except Exception as e:
        raise gr.Error(f"Failed to save customer glossary template: {e}") from e


def reset_customer_glossary_template():
    try:
        restored_path, restored_rows = restore_customer_glossary_template_rows()
        message = _("Configuration saved to: {path}").format(path=restored_path)
        return gr.update(value=restored_rows), gr.update(value=message, visible=True)
    except Exception as e:
        raise gr.Error(f"Failed to reset customer glossary template: {e}") from e
...
def build_ui_inputs(*args):
    ...
    Args:
        *args: UI setting controls in the following order:
            ...
            prompt, min_text_length, rpc_doclayout, custom_system_prompt_input, use_builtin_fashion_prompt, glossary_file,
            use_builtin_fashion_glossary, save_auto_extracted_glossary, enable_auto_term_extraction, primary_font_family, skip_clean,
    ...
    fixed_param_names = [
        ...
        "use_builtin_fashion_prompt",
        "glossary_file",  # will be converted to glossaries
        "use_builtin_fashion_glossary",
        "save_auto_extracted_glossary",
    ...
    ui_inputs["brand_name"] = _resolve_brand_name(ui_inputs.get("brand_name"))
    ui_inputs["brand_url"] = _resolve_brand_url(ui_inputs.get("brand_url"))
...
with gr.Blocks(
    title=_resolve_brand_name(settings.gui_settings.brand_name),
    ...
    with Translate(get_translation_dic(translation_file_path), lang_selector):
        header_markdown = gr.Markdown(
            _build_brand_markdown(
                settings.gui_settings.brand_name,
                settings.gui_settings.brand_url,
            )
        )
        ...
        llm_only_inputs = []
        glossary_support_inputs = []
        glossary_preview_inputs = []
            ...
            with gr.Column(scale=1):
                ...
                with gr.Group(visible=False, elem_classes=["settings-container"]) as tab_settings:
                    ...
                    brand_name_input = gr.Textbox(
                        label=_("Brand Name"),
                        value=_resolve_brand_name(settings.gui_settings.brand_name),
                    )
                    brand_url_input = gr.Textbox(
                        label=_("Brand Link"),
                    ...
                    with gr.Group() as translation_engine_settings:
                        ...
                        for service_name in available_services:
                            ...
                            ] = service_supports_builtin_fashion_glossary(
                                metadata.translate_engine_type,
                                ...
                                for (
                                    ...
                                    if gui_extra.get("widget") == "dropdown":
                                        field_input = gr.Dropdown(
                                            label=_translate_gui_text(field.description),
                    ...
                    with gr.Accordion(_("Advanced Options"), open=False):
                        ...
                        llm_only_inputs.append(custom_system_prompt_input)
                        use_builtin_fashion_prompt = gr.Checkbox(
                            label=_("Use built-in fashion prompt"),
                            value=not settings.translation.disable_builtin_fashion_prompt,
                            interactive=True,
                            visible=_service_supports_llm(service.value),
                        )
                        llm_only_inputs.append(
                            use_builtin_fashion_prompt
                        )
                        ...
                        glossary_file = gr.File(
                            ...
                            file_types=[".csv"],
                            type="binary",
                            visible=_service_supports_fashion_glossary(service.value),
                        )
                        glossary_support_inputs.append(glossary_file)
                        use_builtin_fashion_glossary = gr.Checkbox(
                            label=_("Use built-in fashion glossary"),
                            value=not settings.translation.disable_builtin_fashion_glossary,
                            interactive=True,
                            visible=_service_supports_fashion_glossary(service.value),
                        )
                        glossary_support_inputs.append(
                            use_builtin_fashion_glossary
                        )
                        ...
                        glossary_preview_inputs.append(glossary_table)

                        gr.Markdown(_("### Customer Glossary Template"))
                        customer_glossary_table = gr.Dataframe(
                            headers=["source", "target", "tgt_lng"],
                            datatype=["str", "str", "str"],
                            value=_load_customer_glossary_editor_rows(),
                            interactive=True,
                            col_count=(3, "fixed"),
                            row_count=(12, "dynamic"),
                        )
                        with gr.Row():
                            save_customer_glossary_btn = gr.Button(
                                _("Save Customer Glossary Template"),
                                variant="secondary",
                            )
                            reset_customer_glossary_btn = gr.Button(
                                _("Reset Customer Glossary Template"),
                                variant="secondary",
                            )
                        customer_glossary_status = gr.Markdown(visible=False)
        ...
        def on_select_service(service_name, *detail_values):
            ...
            llm_support = _service_supports_llm(service_name)
            glossary_support = _service_supports_fashion_glossary(service_name)
            return_list = []
            llm_updates = [
                gr.update(visible=llm_support)
                for i in range(len(llm_only_inputs))
            ]
            glossary_support_updates = [
                gr.update(visible=glossary_support)
                for i in range(len(glossary_support_inputs))
            ]
            glossary_preview_updates = [
                gr.update(visible=glossary_support)
                for i in range(len(glossary_preview_inputs))
            ]
        ...
        def on_glossary_file_change(glossary_file, service_name):
            glossary_list = _load_glossary_preview_rows(
                _get_default_gui_glossary_files(
                    service_name,
                    settings.translation.glossaries,
                )
            )
            if glossary_file is None:
                return gr.update(
                    visible=bool(glossary_list),
                    value=glossary_list if glossary_list else None,
                )
        ...
        def update_brand_header(brand_name, brand_url):
            return gr.update(
                value=_build_brand_markdown(brand_name, brand_url)
            )

        brand_name_input.change(
            update_brand_header,
            inputs=[brand_name_input, brand_url_input],
            outputs=[header_markdown],
        )
        brand_url_input.change(
            update_brand_header,
            inputs=[brand_name_input, brand_url_input],
            outputs=[header_markdown],
        )
        ...
        on_select_service_outputs = (
            llm_only_inputs + glossary_support_inputs + glossary_preview_inputs + detail_text_inputs
        )
        ...
        glossary_file.change(
            ...
            [glossary_file, service],
            outputs=glossary_table,
        )

        save_customer_glossary_btn.click(
            save_customer_glossary_template,
            inputs=[customer_glossary_table],
            outputs=[customer_glossary_table, customer_glossary_status],
        )
        reset_customer_glossary_btn.click(
            reset_customer_glossary_template,
            outputs=[customer_glossary_table, customer_glossary_status],
        )
            ...
            try:
                ...
                for metadata in TRANSLATION_ENGINE_METADATA:
                    if getattr(fresh_settings, metadata.cli_flag_name, False):
                        ...
                        break
                if not selected_service or selected_service not in available_services:
                    selected_service = default_gui_service
                llm_support = _service_supports_llm(selected_service)
                glossary_support = _service_supports_fashion_glossary(
                    selected_service
                )
                ...
                updates.append(
                    gr.update(
                        value=_resolve_brand_name(fresh_settings.gui_settings.brand_name)
                    )
                )
                updates.append(
                    gr.update(
                ...
                updates.append(
                    gr.update(
                        value=not fresh_settings.translation.disable_builtin_fashion_prompt,
                        visible=llm_support,
                    )
                )
                updates.append(
                ...
                updates.append(
                    gr.update(
                        value=not fresh_settings.translation.disable_builtin_fashion_glossary,
                        visible=glossary_support,
                    )
                )
                updates.append(
                    gr.update(
                ...
                glossary_preview_rows = _load_glossary_preview_rows(
                    _get_default_gui_glossary_files(
                        selected_service,
                        fresh_settings.translation.glossaries,
                    )
                )
                updates.append(
                    gr.update(
                        value=glossary_preview_rows if glossary_preview_rows else None,
                        visible=glossary_support and bool(glossary_preview_rows),
                    )
                )  # glossary_table visibility
                updates.append(
                    gr.update(value=_load_customer_glossary_editor_rows())
                )  # customer_glossary_table value
                updates.append(
                    gr.update(value="", visible=False)
                )  # customer_glossary_status visibility/value
                updates.append(
                    gr.update(
    ...
    for candidate_port in range(preferred_port, preferred_port + max_attempts):
        if _can_bind_port("127.0.0.1", candidate_port) and _can_bind_port(
            ...
            if candidate_port != preferred_port:
                logger.warning(
                    "Port %s is already in use, falling back to %s",
                    preferred_port,
                    candidate_port,
                )
...
def setup_gui(
    ...
    if resolved_server_port != server_port:
        print(
