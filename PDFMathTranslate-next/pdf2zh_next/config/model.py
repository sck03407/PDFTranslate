...
LOCAL_GLOSSARY_COMPATIBLE_ENGINES: set[str] = set()
DEFAULT_GUI_BRAND_NAME = "PDFTranslate"
DEFAULT_GUI_BRAND_URL = ""


def service_supports_builtin_fashion_glossary(
    service_name: str | None,
    *,
    support_llm: bool = False,
) -> bool:
    return support_llm or service_name in LOCAL_GLOSSARY_COMPATIBLE_ENGINES


def service_supports_glossary(
    service_name: str | None,
    *,
    support_llm: bool = False,
) -> bool:
    return service_supports_builtin_fashion_glossary(
        service_name,
...
class GUISettings(BaseModel):
    ...
    brand_name: str = Field(
        default=DEFAULT_GUI_BRAND_NAME,
        description="Custom brand name shown in the GUI header",
    )
    brand_url: str = Field(
...
class TranslationSettings(BaseModel):
    ...
    disable_builtin_fashion_glossary: bool = Field(
        default=False,
        description="Disable the bundled fashion apparel glossary.",
    )
    disable_builtin_fashion_prompt: bool = Field(
        default=False,
        description="Disable the bundled fashion translation system prompt.",
    )
    ...
    def validate_settings(self) -> None:
        ...
        if self.term_extraction_engine_settings is not None:
            ...
            if main_metadata and main_metadata.support_llm:
                ...
                if not service_supports_builtin_fashion_glossary(main_engine_type):
                    self.translation.disable_builtin_fashion_glossary = True
                    log.warning(
                        "Current translation engine does not support LLM, "
                        "the bundled fashion glossary will be disabled."
                    )
        ...
        if self.translate_engine_settings.translate_engine_type == "SiliconFlowFree":
            ...
            if not self.translation.no_auto_extract_glossary:
                self.translation.no_auto_extract_glossary = True
                log.warning(
                    "SiliconFlowFree does not reliably support automatic term extraction JSON responses, "
                    "automatic term extraction has been disabled."
                )
