...
class CustomOpenAICompatibleSettings(BaseModel):
    """Custom third-party OpenAI-compatible service settings"""

    translate_engine_type: Literal["CustomOpenAICompatible"] = Field(
        default="CustomOpenAICompatible"
    )
    support_llm: Literal["yes", "no"] = Field(
        default="yes", description="Whether the translator supports LLM"
    )

    custom_openai_compatible_name: str = Field(
        default="My Custom Service",
        description="Custom third-party service name",
    )
    custom_openai_compatible_model: str = Field(
        default="gpt-4o-mini", description="Custom third-party model to use"
    )
    custom_openai_compatible_base_url: str | None = Field(
        default=None, description="Custom third-party API base URL"
    )
    custom_openai_compatible_api_key: str | None = Field(
        default=None, description="Custom third-party API key"
    )
    custom_openai_compatible_timeout: str | None = Field(
        default=None, description="Timeout (seconds) for custom third-party service"
    )
    custom_openai_compatible_temperature: str | None = Field(
        default=None, description="Temperature for custom third-party service"
    )
    custom_openai_compatible_reasoning_effort: str | None = Field(
        default=None,
        description="Reasoning effort for custom third-party service (minimal/low/medium/high)",
    )
    custom_openai_compatible_send_temperature: bool | None = Field(
        default=None,
        description="Send temperature to custom third-party service",
    )
    custom_openai_compatible_send_reasoning_effort: bool | None = Field(
        default=None,
        description="Send reasoning effort to custom third-party service",
    )
    custom_openai_compatible_enable_json_mode: bool | None = Field(
        default=None,
        description="Enable JSON mode for custom third-party service",
    )

    def validate_settings(self) -> None:
        if not self.custom_openai_compatible_api_key:
            raise ValueError("Custom third-party API key is required")
        if not self.custom_openai_compatible_base_url:
            raise ValueError("Custom third-party API base URL is required")
        if not self.custom_openai_compatible_model:
            raise ValueError("Custom third-party model is required")
        self.custom_openai_compatible_name = _clean_string(
            self.custom_openai_compatible_name
        ) or "My Custom Service"
        self.custom_openai_compatible_api_key = _clean_string(
            self.custom_openai_compatible_api_key
        )
        self.custom_openai_compatible_base_url = _clean_url(
            self.custom_openai_compatible_base_url
        )
        self.custom_openai_compatible_model = _clean_string(
            self.custom_openai_compatible_model
        )
        self.custom_openai_compatible_timeout = _check_if_positive_float(
            _clean_string(self.custom_openai_compatible_timeout), field="Timeout"
        )
        self.custom_openai_compatible_temperature = _clean_string(
            self.custom_openai_compatible_temperature
        )
        self.custom_openai_compatible_reasoning_effort = _clean_string(
            self.custom_openai_compatible_reasoning_effort
        )
        if self.custom_openai_compatible_send_temperature:
            if not self.custom_openai_compatible_temperature:
                raise ValueError(
                    "Temperature is required when send temperature is enabled"
                )
            try:
                float(self.custom_openai_compatible_temperature)
            except ValueError as e:
                raise ValueError("Temperature must be a float") from e
        if (
            self.custom_openai_compatible_send_reasoning_effort
            and not self.custom_openai_compatible_reasoning_effort
        ):
            raise ValueError(
                "Reasoning effort is required when send reasoning effort is enabled"
            )
    ...
    def transform(self) -> OpenAISettings:
        return OpenAISettings(
            openai_model=self.custom_openai_compatible_model,
            openai_api_key=self.custom_openai_compatible_api_key,
            openai_base_url=self.custom_openai_compatible_base_url,
            openai_timeout=self.custom_openai_compatible_timeout,
            openai_temperature=self.custom_openai_compatible_temperature,
            openai_reasoning_effort=self.custom_openai_compatible_reasoning_effort,
            openai_send_temprature=self.custom_openai_compatible_send_temperature,
            openai_send_reasoning_effort=self.custom_openai_compatible_send_reasoning_effort,
            openai_enable_json_mode=self.custom_openai_compatible_enable_json_mode,
        )


GUI_PASSWORD_FIELDS.append("custom_openai_compatible_api_key")
GUI_SENSITIVE_FIELDS.append("custom_openai_compatible_base_url")
...
TRANSLATION_ENGINE_SETTING_TYPE: TypeAlias = (
    ...
    | CustomOpenAICompatibleSettings
    | ClaudeCodeSettings
    | CLISettings
)
